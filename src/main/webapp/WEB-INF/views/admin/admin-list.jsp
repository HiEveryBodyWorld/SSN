...
<div class="Hui-article">
		<article class="cl pd-20">
		    <form id="list_form" action="/sys/admin/tolist" method="post">
		...
		<input type="text" onfocus="WdatePicker({ minDate:'#F{$dp.$D(\'datemin\')}',maxDate:'%y-%M-%d' })" id="datemax" name="endDate" class="input-text Wdate" style="width:120px;">
				<input type="text" class="input-text" style="width:250px" placeholder="输入管理员名称" id="account" name="account">
				...
				<tbody>
						<c:forEach var="entity" items="${page.rows}" varStatus="vs">
							<tr class="tdChecked">
								...
								<td class="td-status">
								<c:if test="${entity.isDel==1 }">
								<span class="label label-success radius">已启用</span>
								</c:if>
								<c:if test="${entity.isDel==0 }">
								<span class="label label-default radius">已禁用</span>
								</c:if>
								</td>
								<td class="td-manage">
								<c:if test="${entity.isDel==1 }">
								<a onClick="admin_stop(this,${entity.id})" href="javascript:;" title="停用" style="text-decoration:none"><i class="Hui-iconfont">&#xe631;</i></a>
								</c:if>
								<c:if test="${entity.isDel==0 }">
								<a onClick="admin_start(this,${entity.id})" href="javascript:;" title="启用" style="text-decoration:none"><i class="Hui-iconfont">&#xe615;</i></a>
								</c:if>
								<a title="编辑" href="javascript:;" onclick="admin_edit('管理员编辑','toedit/+${entity.id}','800','500')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>
								<a title="删除" href="javascript:;" onclick="admin_del(this,'${entity.id}')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
							</tr>
						</c:forEach>
				</tbody>
			</table>
...
function admin_stop(obj,id){
	layer.confirm('确认要停用吗？',function(index){
        ...
        var url ='/sys/admin/update?isDel=0&id='+id
		jQuery.ajax({
	        type : 'POST',
	        contentType : 'application/json',
	        url : url,
...
function admin_start(obj,id){
	layer.confirm('确认要启用吗？',function(index){
		//此处请求后台程序，下方是成功后的前台处理……
        var url ='/sys/admin/update?isDel=1&id='+id
		jQuery.ajax({
	        type : 'POST',
	        contentType : 'application/json',
	        url : url,

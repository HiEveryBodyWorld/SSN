...
<article class="page-container">
	...
	<input name="id" type="hidden" value='${resources.id}'>
	<div class="row cl">
		<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>权限上级：</label>
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" value="${resources.pName}" placeholder="" id="pId" name="pId" disabled="disabled">
		</div>
	</div>
	<div class="row cl">
		...
		<span class="select-box">
		  <select class="select" size="1" name="type">
		   <c:if test="${resources.id == null }">
			<option value="0">菜单</option>
			<option value="1">动作</option>
			</c:if>
			<c:if test="${resources.id != null }">
				<option value="0" ${resources.type == 0 ? 'selected="selected"' : '' }>菜单</option>
				<option value="1" ${resources.type == 1 ? 'selected="selected"' : '' }>动作</option>
			</c:if>
		  </select>
		</span>
		</div>
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" value="${resources.name}" placeholder="" id="name" name="name">
		</div>
	</div>
	<div class="row cl">
		<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>权限地址：</label>
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text"  value="${resources.url}" name="url" id="url">
		</div>
	</div>
	<div class="row cl">
...
$(function(){
	...
	$("#form-admin-edit").validate({
		...
		submitHandler:function(form){
			var url ='/sys/permissions/update';
			$(form).ajaxSubmit({
				type: 'post',
				url: url ,
